...
def deduplicate_activities(activities: List[Activity]) -> List[Activity]:
    ...
    for activity in activities:
        if activity.start == activity.end:
            continue
        if is_operation_signature_garbage(activity.text):
            continue
...
def _extract_pdf_text(data: bytes) -> str:
    """Extrae texto de PDF; usa OCR si hace falta o si la boleta Weatherford no parsea."""
    text = ""
    try:
        import pdfplumber
        pages = []
    ...
    if is_weatherford_boleta(text) and not parse_weatherford_boleta(text):
        ocr_text = ocr_pdf_bytes(data)
        if parse_weatherford_boleta(ocr_text) or len(re.sub(r"\s+", "", ocr_text)) > stripped_len:
            return ocr_text
...
def force_24h_operation_if_summary_only(report: Dict[str, str], raw_text: str) -> Dict[str, str]:
    ...
    activities = [a for a in activities if not is_operation_signature_garbage(a.text)]
    ...
    if activities or (already_has_range and not is_operation_signature_garbage(operation)):
        return report
...
def is_weatherford_boleta(text: str) -> bool:
    return bool(
        re.search(
            r"WEATHERFORD|Boleta\s+de\s+Campo|Tiempos\s+de\s+Operaci[oó]n|Volumetr[ií]a\s+para\s+Cobro",
            text,
            re.IGNORECASE,
        )
    )
...
def is_operation_signature_garbage(body: str) -> bool:
    """Detecta pie de firma OCR en lugar de actividad real de perforación."""
    body = clean_text(body)
    if len(body) < 8:
        return True
    if re.search(
        r"perfora|conexi[oó]n|circula|survey|repasa|bomba|lodo|\brpm\b|\bwob\b|\brop\b|"
        r"condiciones\s+de\s+operaci[oó]n|tiempos\s+diacretizados|hthp|viscosidad",
        body,
        re.IGNORECASE,
    ):
        return False
    return bool(
        re.search(
            r"representa|compa[nñ]i[aá]|ficha|firma|nombre.*ficha|ing\.?\s*operaciones|"
            r"volumetr[ií]a\s+para\s+cobro|membre",
            body,
            re.IGNORECASE,
        )
    )
...
def normalize_weatherford_ocr_text(text: str) -> str:
    """Normaliza horas y espacios típicos del OCR en boletas Weatherford/Pemex."""
    text = text.replace("\r", "\n")
    text = re.sub(r"(?<=\d)\.(?=\d{2}\b)", ":", text)
    text = re.sub(r"\b(\d{1,2})\s+(\d{2})\s+(\d{1,2})\s+(\d{2})\s+(\d{1,2})\s+(\d{2})\s+(\d{3,5})\b", r"\1:\2 \3:\4 \5:\6 \7", text)
    return text
...
def parse_standard_operations(text: str) -> List[Activity]:
    ...
    for index, match in enumerate(matches):
        ...
        if body and not is_operation_signature_garbage(body):
...
def clean_weatherford_activity_text(body: str) -> str:
    ...
    body = clean_text(body)
    body = re.sub(r"\bVolumetr[ií]a\s+para\s+Cobro\b.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"\bComentarios\b.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"\bRHA\s+DE\s+SALIDA\b.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"\bBHA\s+DE\s+SALIDA\b.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"Nombre\s*,?\s*Ficha\s*y\s*Firma.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"Nombre\.?Ficha\s*y\s*Firma.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"FichayFirma.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"membre\s*,?\s*Ficha.*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"\(\s*ing\.?\s*Operaciones\s*\).*", "", body, flags=re.IGNORECASE | re.DOTALL)
    body = re.sub(r"\s{2,}", " ", body)
    return clean_text(body)


def _weatherford_activity_keywords(body: str) -> bool:
    return bool(
        re.search(
            r"PERFORA|CONEXI[oó]N|CIRCULA|SURVEY|MUESTRA|REPASA|ROP|RPM|WOB|BOMBA|LODO|GAS|"
            r"RECUPERACI[oó]N|CONDICIONES\s+DE\s+OPERACI[oó]N|TIEMPOS\s+DIACRETIZADOS",
            body,
            re.IGNORECASE,
        )
    )
...
def _parse_weatherford_time_rows(compact: str) -> List[Activity]:
    """Extrae filas De|A|Horas|Prof|Sarta|Actividad de la tabla Tiempos de Operación."""
    row_pattern = re.compile(
        r"(?P<start>\b\d{1,2}[:\.]\d{2})\s+"
        r"(?P<end>\d{1,2}[:\.]\d{2})\s+"
        r"(?P<hours>\d{1,2}[:\.]\d{2})\s+"
        r"(?P<depth>\d{3,5})\s+"
        r"(?:(?P<sarta>\d{1,2})\s+)?"
        r"(?P<body>.*?)(?="
        r"\b\d{1,2}[:\.]\d{2}\s+\d{1,2}[:\.]\d{2}\s+\d{1,2}[:\.]\d{2}\s+\d{3,5}\b|"
        r"Volumetr[ií]a\s+para\s+Cobro|\bComentarios\b|RHA\s+DE\s+SALIDA|BHA\s+DE\s+SALIDA|"
        r"Representante|Nombre\s*,?\s*Ficha|$)",
    ...
    for match in row_pattern.finditer(compact):
        ...
        body = clean_weatherford_activity_text(match.group("body"))
        if len(body) < 8 or not _weatherford_activity_keywords(body):
            continue
        if is_operation_signature_garbage(body):
            continue
...
def _parse_weatherford_perf_blocks(compact: str) -> List[Activity]:
    """Respaldo: asocia bloques PERFORA con filas de tiempo detectadas."""
    time_rows = list(
        re.finditer(
            r"(?P<start>\b\d{1,2}[:\.]\d{2})\s+(?P<end>\d{1,2}[:\.]\d{2})\s+(?P<hours>\d{1,2}[:\.]\d{2})\s+(?P<depth>\d{3,5})",
            compact,
        )
    )
    perf_rows = list(re.finditer(r"\bPERFORA\b", compact, re.IGNORECASE))
    if not time_rows or not perf_rows:
        return []

    activities: List[Activity] = []
    for i, tm in enumerate(time_rows):
        if i >= len(perf_rows):
            break
        start = normalize_time(tm.group("start"))
        end = normalize_time(tm.group("end"))
        if end == "00:00" and start != "00:00":
            end = "24:00"
        body_start = perf_rows[i].start()
        body_end = perf_rows[i + 1].start() if i + 1 < len(perf_rows) else len(compact)
        body = clean_weatherford_activity_text(compact[body_start:body_end])
        if len(body) >= 8 and _weatherford_activity_keywords(body) and not is_operation_signature_garbage(body):
            activities.append(Activity(start=start, end=end, text=body))
    return activities


def parse_weatherford_boleta(text: str) -> List[Activity]:
    """
    Parser para Boleta de Campo Weatherford/Pemex escaneada.

    La tabla normalmente viene como:
      De | A | Horas | Prof (m) | Sarta # | Actividad
      00:00 04:00 04:00 3663 5 PERFORA...
    """
    if not is_weatherford_boleta(text):
        return []

    section = extract_between_markers(
        text,
        start_markers=[
            r"Tiempos\s+de\s+Operaci[oó]n",
            r"\bDe\s+A\s+Horas\b",
            r"\bProf\s*\(m\)\s+Sarta",
        ],
        stop_markers=[
            r"Volumetr[ií]a\s+para\s+Cobro",
            r"\bComentarios\b",
            r"RHA\s+DE\s+SALIDA",
            r"BHA\s+DE\s+SALIDA",
            r"Representante\s+de\s+la\s+comp",
            r"Nombre\s*,?\s*Ficha\s*y\s*Firma",
        ],
    )
    ...
    compact = normalize_weatherford_ocr_text(section)
    compact = re.sub(r"[ \t]+", " ", compact)
    compact = re.sub(r"\n+", " ", compact)

    activities = _parse_weatherford_time_rows(compact)
    if not activities:
        activities = _parse_weatherford_perf_blocks(compact)

    return normalize_activity_sequence(deduplicate_activities(activities))


def split_activity_text(text: str) -> List[Activity]:
    if is_weatherford_boleta(text):
        activities = parse_weatherford_boleta(text)
        if activities:
...
def find_weatherford_header_value(text: str, label: str, default: str = "") -> str:
    """Extrae valores de encabezados de boletas Weatherford con OCR irregular."""
    label_re = re.escape(label).replace(r"\ ", r"\s+")
    pattern = rf"{label_re}\s*[:\-]?\s*([A-Za-z0-9ÁÉÍÓÚÜÑáéíóúüñ()./#_\-\s]+?)(?=\s+(?:Cliente|Plataforma|Pozo|Campo|No\.\s*Contrato|Etapa|Fecha\s+de\s+inicio|Fecha\s+actual|REPORTE|PERFORACI[oó]N|CEMENTO|Categor[ií]a)\b|\n|$)"
    match = re.search(pattern, text, re.IGNORECASE)
    if not match:
        return default
    value = clean_text(match.group(1))
...
def improve_weatherford_fields(report: Dict[str, str], blob: str, uploaded_name: str = "") -> Dict[str, str]:
    """Corrige campos cuando el PDF es una Boleta de Campo Weatherford/Pemex."""
    if not re.search(r"WEATHERFORD|Boleta\s+de\s+Campo|Tiempos\s+de\s+Operaci[oó]n", blob, re.IGNORECASE):
        return report

    cliente = find_weatherford_header_value(blob, "Cliente")
    pozo = find_weatherford_header_value(blob, "Pozo")
    campo = find_weatherford_header_value(blob, "Campo")
    etapa = find_weatherford_header_value(blob, "Etapa")
    fecha = find_weatherford_header_value(blob, "Fecha actual") or find_weatherford_header_value(blob, "FECHA")
    contrato = find_weatherford_header_value(blob, "No. Contrato")
    ...
    if pozo:
        ...
        report["pozo"] = campo
    if etapa:
        report["operacion_actual"] = etapa
    if fecha:
        report["fecha"] = normalize_date(fecha)
    if contrato:
    ...
    if m:
        ...
        depths = [int(x) for x in re.findall(r"\b(?:Prof\s*\(m\)|Profundidad)\D{0,25}(\d{3,5})\b", blob, re.IGNORECASE)]
        if depths:
            report["profundidad"] = str(max(depths))

    if not report.get("compania") or len(report.get("compania", "")) > 60:
...
def extract_report(raw_text: str, uploaded_name: str = "", email_body: Optional[str] = None) -> Dict[str, str]:
    ...
    return improve_weatherford_fields(report, blob, uploaded_name)
            ...
            with right:
                ...
                if raw_text:
                    if report.get("_operacion_cuerpo_correo"):
                        ...
                        st.info(
                            "Se extrae la tabla **Tiempos de Operación** (Weatherford/Pemex) o "
                            "**Daily Activity Summary** (Baker). "
